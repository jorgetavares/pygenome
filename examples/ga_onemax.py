...
def steady_state_no_elitism():
    np.random.seed(42)
    pop = pg.genetic_algorithm_binary(
        fitness_fn, onemax_size, generational=False, total_generations=200)
    best = pg.best_individual(pop)
    print('fitness: %s\tgenotype: %s' % (best.fitness.value, best.genotype))
...
def steady_state_with_elitism():
    np.random.seed(42)
    pop = pg.genetic_algorithm_binary(
        fitness_fn, onemax_size, generational=False, total_generations=200, elitism=True)
    best = pg.best_individual(pop)
    print('fitness: %s\tgenotype: %s' % (best.fitness.value, best.genotype))
