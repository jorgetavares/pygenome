...
def test_uniform_chromossome():
    ...
    np.random.seed(5389)
    chromossome = individual.uniform_chromossome(10)
    assert np.size(chromossome) == 10
...
def test_normal_chromossome():
    ...
    np.random.seed(5389)
    chromossome = individual.normal_chromossome(10)
    assert np.size(chromossome) == 10
    # with mean/sigma values, N(mean,sigma)
    np.random.seed(5389)
    chromossome = individual.normal_chromossome(10, mean=-5.0, sigma=5.0)
    assert np.size(chromossome) == 10
