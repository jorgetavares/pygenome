...
class PrimitiveSet(object):
    ...
    def addFunction(self, fn, arity, types=None):
        ...
        if fn not in self.primitives:
            ...
            if self.typed:
                self._addTypesCache(self.num_primitives,
                                    types, self.functions_types)
    ...
    def addVariable(self, var, types=None):
        if self.typed and types is None:
            raise AttributeError(
                'This is a typed variable set so types are required!')
...
def grow_tree(pset, max_depth, total_max_depth, initial_type=None):
    ...
    def grow(depth, arg_type=None):
        if depth == 1:
            ...
            if arg_type is None:
                idx = all_terminals_idx[np.random.randint(
                    all_terminals_idx.size)]
            else:
                valid_terminals = []
                if arg_type in pset.terminals_types:
                    valid_terminals = valid_terminals + \
                        pset.terminals_types[arg_type]
                if arg_type in pset.variables_types:
                    valid_terminals = valid_terminals + \
                        pset.variables_types[arg_type]

                if len(valid_terminals) == 0:
                    AttributeError(
                        'Typed Primitive not found in Terminal/Variable Set!')
            ...
            if arg_type is None:
                idx = all_primitives_idx[np.random.randint(
                    all_primitives_idx.size)]
            else:
                valid_set = []
            ...
            if idx in pset.terminals or idx in pset.variables:
                ...
                for a in range(arity):
                    grow.position += 1
                    if pset.typed:
                        # only cares about the arguments types
                        grow(depth, types[a + 1])
                    else:
                        grow(depth)
