...
def apply_global_crossover(pop, cx_op, **kargs):
    ...
    for i in range(0, pop.size):
        pop.individuals[i] = cx_op(pop.individuals[i],
                                   pop.individuals[np.random.randint(
                                       pop.size)],
                                   **kargs)
        pop.individuals[i].run_eval = True
...
def intermediary_crossover(i1, i2):
    ...
    '''
    o1 = pg.Individual(genotype=np.empty(
        i1.genotype.size, dtype=i1.genotype.dtype))
...
def tree_crossover(parent1, parent2, pset=None):
    ...
    # create offsprings
    offspring1 = pg.TreeIndividual(tree=np.zeros(
        parent1.genotype.size, dtype=parent1.genotype.dtype))
    offspring2 = pg.TreeIndividual(tree=np.zeros(
        parent2.genotype.size, dtype=parent2.genotype.dtype))
    ...
    if len1 > parent1.genotype.size:
        ...
        arraycopy(parent2.genotype, start2,
                  offspring1.genotype, start1, num_elements)
    ...
    if len2 > parent2.genotype.size:
        ...
        arraycopy(parent1.genotype, start1,
                  offspring2.genotype, start2, num_elements)
