...
def uncorrelated_one_step_mutation(ind, epsilon=1e-08):
    ...
    sigma = sigma_check(
        chromossome[-1] * np.exp(tau * np.random.normal()), epsilon)
    ...
    # last is the self-adaptive rate, not part of the solution
    offspring[-1] = sigma
...
def uncorrelated_one_step_mutation_adaptive(ind, epsilon=1e-08):
    ...
    sigma = sigma_check(
        ind.parameters[0] * np.exp(tau * np.random.normal()), epsilon)
...
def tree_point_mutation(i1, pset=None, gene_rate=None, **kargs):
    ...
    if pset.ephemeral_cache:
        ...
        terminals_idx = np.concatenate(
            [np.array(terminals_keys), np.array(list(pset.ephemeral_constants.keys()))])
    else:
    ...
    while i < new_genotype.size and new_genotype[i] != 0:
        ...
        if np.random.uniform() < gene_rate:
            ...
            if primitive in pset.terminals or primitive in pset.variables or primitive in pset.ephemeral_constants:
                if pset.typed:
                    _, term_types = pset.terminals[primitive] if primitive in pset.terminals else (
                        None, None)
                    _, ephm_types = pset.ephemeral_constants[primitive] if primitive in pset.ephemeral_constants else (
                        None, None)
                    _, vars_types = pset.variables[primitive] if primitive in pset.variables else (
                        None, None)
                    ...
                    valid_terminals = np.concatenate([np.array(typed_terminals, dtype=int), np.array(
                        typed_variables, dtype=int), np.array(typed_ephemerals, dtype=int)])
    ...
    new_individual = pg.TreeIndividual(
        tree=new_genotype, depth=i1.depth, nodes=i1.nodes)
...
def subtree_mutation(parent, pset=None, **kargs):
    ...
    offspring = pg.TreeIndividual(tree=np.zeros(
        parent.genotype.size, dtype=parent.genotype.dtype))
